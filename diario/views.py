...
def home(request):
    textos = Diario.objects.all().order_by('create_at')[:3] #pegando os 3 primeiros dados
...
def escrever(request):
    if request.method == 'GET':
        pessoas = Pessoa.objects.all()       # pegando todas as pessoas 
        ...
        if len(titulo.strip()) == 0 or len(texto.strip()) == 0:
            ...
            messages.error(request, 'O título e o texto não podem estar vazios.') 
        ...
        messages.success(request, 'Diário registrado com sucesso!')
        return redirect('escrever')
...
def dia(request):
    ...
    data_formatada = datetime.strptime(data, '%Y-%m-%d') #transformando string em variavel de tipo data
    diarios = Diario.objects.filter(create_at__gte=data_formatada).filter(create_at__lte=data_formatada+timedelta(days=1)) #gte = maior ou igual, lte = menor ou igual
